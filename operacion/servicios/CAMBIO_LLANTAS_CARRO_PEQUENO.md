# Cambio de llantas — carro pequeño

## Propósito

Registrar los servicios y tarifas que Joel confirma para trabajos relacionados con cambio de llantas en carros pequeños.

## Estado

**Estado general:** `confirmado`  
**Origen:** información directa proporcionada por Joel durante VN-01.  
**Fecha de registro:** 2026-09-20.

## Servicios y tarifas confirmadas — franja Día

| Servicio | Precio | Unidad de cobro |
|---|---:|---|
| Armar y desarmar la llanta | $2.00 | Pendiente de precisar |
| Cambio de posición de llanta | $1.00 | Por llanta |
| Desarmar llantas fuera del vehículo | $1.00 | Pendiente de precisar |
| Armar y desarmar para virarla en el mismo aro | $2.00 | Pendiente de precisar |
| Cambio de llanta por la llanta de emergencia | $2.00 | Pendiente de precisar |
| Cambio por llantas nuevas | $2.00 | Por aro |

## Alcance confirmado

Estas tarifas corresponden a **carro pequeño** y a la **franja Día**. Las tarifas de Noche y los límites horarios de las franjas permanecen pendientes de definición por Joel.

La resolución horaria canónica se mantiene en `operacion/tarifas/TARIFAS_POR_FRANJA.md`.

## Información pendiente

Todavía no se ha confirmado:

- si "armar y desarmar la llanta" se cobra por llanta, aro u operación;
- la unidad exacta de cobro para "desarmar llantas fuera del vehículo";
- la unidad exacta de cobro para "armar y desarmar para virarla en el mismo aro";
- la unidad exacta de cobro para "cambio de llanta por la llanta de emergencia";
- si existen variaciones por tamaño de aro;
- si existen recargos por tipo de llanta;
- materiales incluidos;
- tiempo aproximado de ejecución.

No deben inferirse estos datos hasta que Joel los confirme.

## Regla operativa — cambio de posición

Cuando Joel indica un **cambio de posición de una llanta**, la operación implica mover también la llanta del lado opuesto para efectuar el intercambio.

Por lo tanto:

- la tarifa continúa siendo **$1.00 por llanta**;
- un cambio de posición entre dos ubicaciones implica **2 llantas cobradas**;
- el valor del cambio de posición en ese caso es **$2.00**.

Esta regla fue aclarada directamente durante la operación el 2026-09-20.

Si Joel describe una maniobra distinta que no implique dos llantas, debe aclararse antes de calcular la proforma.
